...
Skill 给 AI Agent 提供使用说明 —— AI 通过 bash 直接运行 `lookin-cli` 命令，更轻量，无需 MCP 配置。一条命令自动检测你的 AI Agent 并安装 Skill：

```bash
curl -fsSL https://raw.githubusercontent.com/a573412261/lookin-cli/main/skills/install.sh | sh
```

支持平台：Claude Code、Codex CLI、Gemini CLI、Goose、OpenCode、Cursor、Windsurf、Cline、Roo Code，以及通用路径 `~/.agents/skills/`。
